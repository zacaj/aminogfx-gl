 ...
 *
 */
class AminoGfxWin : public AminoGfx {
public:
    AminoGfxWin(): AminoGfx(getFactory()->name) {
        //empty
    }
     ...
     */
    static AminoGfxWinFactory* getFactory() {
        static AminoGfxWinFactory *instance = NULL;

        if (!instance) {
            instance = new AminoGfxWinFactory(New);
        }
    ...
    static NAN_MODULE_INIT(Init) {
        AminoGfxWinFactory *factory = getFactory();
...
private:
    ...
    static std::map<GLFWwindow *, AminoGfxWin *> *windowMap;

    //GLFW window
    GLFWwindow *window = NULL;

    static AminoGfxWin* windowToInstance(GLFWwindow *window) {
        std::map<GLFWwindow *, AminoGfxWin *>::iterator it = windowMap->find(window);
    ...
    void initRenderer() override {
        ...
        //store
        windowMap->insert(std::pair<GLFWwindow *, AminoGfxWin *>(window, this));
    ...
    static void handleKeyEvents(GLFWwindow *window, int key, int scancode, int action, int mods) {
        ...
        AminoGfxWin *obj = windowToInstance(window);
    ...
    void atlasTextureHasChanged(texture_atlas_t *atlas) override {
        ...
        //run on main thread
        enqueueJSCallbackUpdate(static_cast<jsUpdateCallback>(&AminoGfxWin::atlasTextureHasChangedHandler), NULL, atlas);
    }
    ...
    AminoVideoPlayer *createVideoPlayer(AminoTexture *texture, AminoVideo *video) override {
        return new AminoWinVideoPlayer(texture, video);
    }
};

//static initializers
bool AminoGfxWin::glfwInitialized = false;
std::map<GLFWwindow *, AminoGfxWin *> *AminoGfxWin::windowMap = new std::map<GLFWwindow *, AminoGfxWin *>();
 ...
 */
AminoGfxWinFactory::AminoGfxWinFactory(Nan::FunctionCallback callback): AminoJSObjectFactory("AminoGfx", callback) {
    //empty
}
 ...
 */
AminoJSObject* AminoGfxWinFactory::create() {
    return new AminoGfxWin();
}
...
AminoWinVideoPlayer::AminoWinVideoPlayer(AminoTexture *texture, AminoVideo *video): AminoVideoPlayer(texture, video) {
#if 0
    //semaphore
...
AminoWinVideoPlayer::~AminoWinVideoPlayer() {
#if 0
    closeDemuxer();
 ...
 */
bool AminoWinVideoPlayer::initStream() {
    //get file name
 ...
 * Demuxer thread.
 */
void AminoWinVideoPlayer::demuxerThread(void *arg) {
    AminoWinVideoPlayer *player = static_cast<AminoWinVideoPlayer *>(arg);
 ...
 * Init demuxer.
 */
void AminoWinVideoPlayer::initDemuxer() {
#if 0
    assert(demuxer);
 ...
 */
void AminoWinVideoPlayer::closeDemuxer() {
#if 0
    //stop playback
    stopPlayback();
 ...
 */
void AminoWinVideoPlayer::initVideoTexture() {
    if (DEBUG_VIDEOS) {
 ...
 * Init texture.
 */
bool AminoWinVideoPlayer::initTexture() {
 ...
 */
void AminoWinVideoPlayer::updateVideoTexture(GLContext *ctx) {
#if 0
    uv_mutex_lock(&frameLock);
 ...
 */
double AminoWinVideoPlayer::getMediaTime() {
    if (playing || paused) {
        return mediaTime;
    }
 ...
 */
double AminoWinVideoPlayer::getDuration() {
    if (demuxer) {
 ...
 */
double AminoWinVideoPlayer::getFramerate() {
    if (demuxer) {
        return demuxer->fps;
    }
 ...
 * Stop playback.
 */
void AminoWinVideoPlayer::stopPlayback() {
    if (!playing && !paused) {
        return;
    }
 ...
 * Pause playback.
 */
bool AminoWinVideoPlayer::pausePlayback() {
    if (!playing) {
        return true;
    }
 ...
 */
bool AminoWinVideoPlayer::resumePlayback() {
    if (!paused) {
        return true;
    }
